...
import { _getrightOperation, _queryOrgUserTree, _getDailyCallForm, _getDailyCallForm2, _exportPlayback, _getColumns } from '../../../common/api/reportForms'
...
import styles from './playBackRecordForm.module.less'
...
class PlayBackRecordForm extends React.Component<any, IState> {
    constructor(props: any) {
        ...
        this.state = {
            ...
            workerNumberData: [], // 用户列表
            department: [], // 存放用户ID
            query_id: '',
            total: 0, // 总条数
            pageSize: 50, // 每页条数
            pageIndex: 1, // 当前页数
            showExportModal: false, // 控制导出弹框
            exportValue: 1,
            tableHeight: 0, // 表格的高度
            TreeShowSearch: true
        }
    ...
    public componentDidMount(): void {
        const abnormal = document.querySelectorAll('#playBack')[0]
        this.setState({
            ...
            _queryOrg({
                ...
                if (res && res.result && res.data !== 'null') {
                    ...
                    localMachine.forEach((item: any): void => {
                        treedata.push({ title: item.name, key: item.system_id, value: item.system_id, children: item.children })
                    })
    ...
    public getUser = async (): Promise<void> => {
        ...
        if (res && res.result) {
            ...
            res.data.forEach((item: any): void => {
                treedata.push({ title: item.full_name, value: item.system_id, key: item.system_id, children: item.children })
            })
            this.setState({
    ...
    public chartExport = async (): Promise<void> => {
        if (this.state.dataSource.length !== 0) {
            if (this.state.exportValue === 1) {
                const res: any = await _exportPlayback({
                    query_id: this.state.query_id,
                    page_index: this.state.pageIndex,
                    page_size: this.state.pageSize
                })
                ...
                if (res !== '') {
                    if (window.navigator.msSaveOrOpenBlob) {
                        ...
                        try {
                            ...
                            window.navigator.msSaveOrOpenBlob(blob, `${intl.get('M26P102261011051100P001').d('回放记录报表.xlsx')}`)
                            this.setState({
                        ...
                        link.download = `${intl.get('M26P102261011051100P001').d('回放记录报表.xlsx')}`
    ...
    public render(): React.ReactElement {
        return (
            <div className={styles.main}>
                <div id="playBack" className={styles.wrapper}>
                    <Table
                ...
                >
                    <Row style={{ marginTop: '20px' }}>
                        <Col span={6}>{intl.get('VP9122636110400000009').d('回放时间')}</Col>
                        <Col span={18}>
...
export default PlayBackRecordForm